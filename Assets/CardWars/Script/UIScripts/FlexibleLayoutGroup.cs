...
public class FlexibleLayoutGroup : LayoutGroup
{
    ...
    public override void CalculateLayoutInputHorizontal()
    {
        ...
        if (fitType == FitType.Height || fitType == FitType.FixedColumns)
        {
        ...
        float cellWidth = parentWith / (float)columns - ((spacing.x / (float)columns)*2)  - (padding.left / (float)columns) - (padding.right/ (float) columns);
